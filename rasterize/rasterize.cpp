#ifndef RASTERIZERDRIVER
#define RASTERIZERDRIVER
/*
 */
...
void renderSceneRZ(std::string filename, GraphicsArgs args);

void renderOBJRZ(std::string filename, GraphicsArgs args);



int rasterize(int argc, char *argv[])
{
	GraphicsArgs args;
	...
	if (filename.find(".obj") != -1 ||
		filename.find(".OBJ") != -1)
		renderOBJRZ(filename, args);

	else if (filename.find(".xml") != -1 ||
				filename.find(".XML") != -1)
		renderSceneRZ(filename, args);
...
void renderOBJRZ(std::string filename, GraphicsArgs args)
{
	...
	camDat.position = Vector3D(0,0,-10);
	camDat.gazeDirection = Vector3D(0,0,1);
